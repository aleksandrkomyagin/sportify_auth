...
from sportify_auth.infrastructure.exceptions.crypto.rsa_key_generator import (
	PublicKeyGenerationException,
	RSAGenerationException,
)
...
class RSAKeyGenerator(IKeyGenerator):
	async def generate_rsa(self) -> RSAKeyDTO:
		...
		try:
			...
			n = (
				urlsafe_b64encode(numbers.n.to_bytes((numbers.n.bit_length() + 7) // 8, "big"))
				.rstrip(b"=")
				.decode("utf-8")
			)
			e = (
				urlsafe_b64encode(numbers.e.to_bytes((numbers.e.bit_length() + 7) // 8, "big"))
				.rstrip(b"=")
				.decode("utf-8")
			)
		except (
	...
	async def generate_public_key_from_jwk(self, jwk: dict[str, Any]) -> str:
		try:
			...
			raise PublicKeyGenerationException(
				message="Ошибка генерации публичного ключа RSA"
			) from e
