...
class KafkaMessageProducer(IMessageProducer):
	...
	async def send(self, topic: str, message: dict, key: str | None = None) -> None:
		try:
			...
			raise SendMessageException(
				message="Ошибка при отправке сообщения продюсером"
			) from e
	...
	async def start(self) -> None:
		try:
			...
			raise StartProducerException(
				message="Ошибка при старте продюсера"
			) from e
	...
	async def stop(self) -> None:
		try:
			...
			raise StopProducerException(
				message="Ошибка при остановке продюсера"
			) from e
